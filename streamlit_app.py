...
# --- Constants ---
CARD_SIZE_DEFAULT = 500
SYMBOL_SIZE_DEFAULT = 80
BORDER_THICKNESS_DEFAULT = 3
MARGIN = 20
...
card_size = st.slider("Card size (pixels):", 300, 1000, CARD_SIZE_DEFAULT)
border_thickness = st.slider("Border thickness:", 1, 10, BORDER_THICKNESS_DEFAULT)
...
# --- Draw card automatically in circle ---
def draw_card_auto(symbols, images, size, border):
    ...
    for i, sym_id in enumerate(symbols):
        ...
        card.paste(img, (int(x - SYMBOL_SIZE_DEFAULT//2), int(y - SYMBOL_SIZE_DEFAULT//2)), img.convert("RGBA"))

    return card

# --- Draw card with manual positions & sizes ---
def draw_card_with_positions(symbols, images, positions, sizes, size, border):
    card = Image.new("RGBA", (size, size), (255, 255, 255, 255))
    draw = ImageDraw.Draw(card)
    center = (size // 2, size // 2)
    radius = (size - MARGIN*2) // 2
...
if image_files and len(image_files) >= total_symbols:
    ...
    for card_idx, card_symbols in enumerate(deck):
        ...
        if mode == "Easy":
            card_img = draw_card_auto(card_symbols, images, card_size, border_thickness)
        else:
            center = card_size // 2
            ...
            for i, sym_id in enumerate(card_symbols):
                ...
                pos_x = st.slider(f"X position (symbol {sym_id + 1}, card {card_idx + 1})", MARGIN, card_size - MARGIN, int(default_positions[i][0]), key=f"x_{card_idx}_{i}")
                pos_y = st.slider(f"Y position (symbol {sym_id + 1}, card {card_idx + 1})", MARGIN, card_size - MARGIN, int(default_positions[i][1]), key=f"y_{card_idx}_{i}")
                size_slider = st.slider(f"Size (symbol {sym_id + 1}, card {card_idx + 1})", 20, 120, SYMBOL_SIZE_DEFAULT, key=f"s_{card_idx}_{i}")
